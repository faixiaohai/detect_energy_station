 ...
 * @LastEditTime: 2024-04-02 16:43:15
...
void Match::MatchBoarding(const cv::Mat &image)
{
    ...
    for (auto &contour : contours)
    {
        ...
        if (cv::contourArea(contour) < now_image.rel_rbox.box_rect.size.area() * 1.8) {
            continue;
        }
    ...
    {   if (image_type == MOTIVED) {
            for (auto &box : now_image.pre_bbox) {
                float distance = Kit::ComputeDistance(now_image.rel_rbox.center, box.rect.center);
                if (distance > AdjustNumber::barinformation.in_r * 0.8 && distance < AdjustNumber::barinformation.out_r * 0.8)
                {
            ...
            { 
                for (auto &last_rel_prebox : last_image.pre_ch_box)
                {
                    ...
                    float distance = Kit::ComputeDistance(now_image.rel_rbox.center, boardbox.rect.center);
                    // std::cout << distance << std::endl;
                    if (iou > IOU_THRESOLD && distance > AdjustNumber::barinformation.in_r * 0.8 && distance < AdjustNumber::barinformation.out_r)
                    {
        ...
        std::vector<BoardBox> tmep_and_box;
        for (int i = 0; i < 5; i++) {
            ...
            if (and_bbox.size() == 2)
            {
                ...
                BoardBox and_box = BoardBox(brect, id, SLOT, rel_point);
                tmep_and_box.push_back(and_box);
                and_bbox.clear();
            }
            else if (and_bbox.size() == 3)
            {
                ...
                tmep_and_box.push_back(and_bbox[0]);
            }
            and_bbox.clear();
        }
        if (!tmep_and_box.empty()) {
            std::vector<BoardBox> temp;
            for (auto &box : now_image.rel_bbox) {
                int i = 0;
                for (auto &box_one : tmep_and_box) {
                    if (box.id != box_one.id) {
                        i++;
                    }
                }
                if (i == tmep_and_box.size()) {
                    temp.push_back(box);
                }
            } 
            for (auto &box : tmep_and_box) {
                temp.push_back(box);
            }
            now_image.rel_bbox.clear();
            now_image.rel_bbox = temp;
        }
        std::cout << "当前图像的真大符数量为" << now_image.rel_bbox.size() << std::endl;
        assert(now_image.rel_bbox.size() < 6 );
...
/// @param image 原图像
/// @param type 是否绘制备选框
ImageInformation Match::Run(const cv::Mat &image, std::string type, std::string type_other)
{
    ...
    cv::imshow("展示图", image);
    cv::waitKey(0);
    ImageInformation temp_info = now_image;
